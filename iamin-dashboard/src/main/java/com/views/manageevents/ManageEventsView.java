package com..views.manageevents;

import java.util.Optional;

import com..data.entity.SampleAddress;
import com..data.service.SampleAddressService;
...
@Route(value = "manage-events/:sampleAddressID?/:action?(edit)", layout = MainLayout.class)
@RolesAllowed("ADMIN")
public class ManageEventsView extends Div implements BeforeEnterObserver {
    ...
    private final String SAMPLEADDRESS_EDIT_ROUTE_TEMPLATE = "manage-events/%s/edit";
    ...
    public ManageEventsView(SampleAddressService sampleAddressService) {
        this.sampleAddressService = sampleAddressService;
        addClassNames("manage-events-view");
        ...
        // Configure Grid
        grid.addColumn("street").setAutoWidth(true);grid.addColumn("postalCode").setAutoWidth(true);grid.addColumn("city").setAutoWidth(true);grid.addColumn("state").setAutoWidth(true);grid.addColumn("country").setAutoWidth(true);
        grid.setItems(
                query -> sampleAddressService
                        .list(PageRequest
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                UI.getCurrent().navigate(String.format(
                        SAMPLEADDRESS_EDIT_ROUTE_TEMPLATE, event.getValue().getId()));
            } else {
                clearForm();
                UI.getCurrent().navigate(ManageEventsView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleAddressId.isPresent()) {
            ...
            if (sampleAddressFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(ManageEventsView.class);
            }
        }
    }
